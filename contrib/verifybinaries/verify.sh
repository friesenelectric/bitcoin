#!/usr/bin/env bash
# Copyright (c) 2016 The Bikcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   bikcoincore.org and bikcoin.org and compares them.
...
WORKINGDIR="/tmp/bikcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bikcoincore.org"
HOST2="https://bikcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="bikcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bikcoin-',
...
if [ $? -ne 0 ]; then
   echo "bikcoin.org failed to provide signature file, but bikcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bikcoin.org and bikcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bikcoin Core binary release signing key installed?"
   fi
