...
const char * const BIKCOIN_CONF_FILENAME = "bikcoin.conf";
const char * const BIKCOIN_PID_FILENAME = "bikcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bikcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bikcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bikcoin
    // Mac: ~/Library/Application Support/Bikcoin
    // Unix: ~/.bikcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bikcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bikcoin";
#else
    // Unix
    return pathRet / ".bikcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BIKCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BIKCOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Bikcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bikcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Bikcoin Core developers";
    }
